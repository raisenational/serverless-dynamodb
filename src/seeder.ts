...
import {type BatchWriteItemCommandInput, type BatchWriteItemCommandOutput} from '@aws-sdk/client-dynamodb';
import {type BatchWriteCommandInput, type BatchWriteCommandOutput} from '@aws-sdk/lib-dynamodb';
 ...
 */
async function writeSeedBatch(dynamodbWriteFunction: DynamoDBWriteFunction, tableName: string, seedValues: object[]) {
	const params = {
		RequestItems: {
...
export async function writeSeeds(dynamodbWriteFunction: DynamoDBWriteFunction, tableName: string, seedValues: object[]) {
	...
	await Promise.all(seedChunks.map(async (chunk) => writeSeedBatch(dynamodbWriteFunction, tableName, chunk)));
}
