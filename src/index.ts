...
import {
	DynamoDBClient, CreateTableCommand, BatchWriteItemCommand, type CreateTableCommandInput,
...
import {type Config, type SeedConfig} from './config';
...
class ServerlessDynamoDBPlugin implements Plugin {
	...
	constructor(private readonly serverless: Serverless, options: Serverless.Options) {
   ...
   */
	...
	async migrateHandler() {
		if (this.shouldExecute()) {
			...
			await Promise.all(this.tableDefinitions.map(async (table) => this.createTable(dynamodb, table)));
			return;
		}
	...
	async seedHandler() {
		if (this.shouldExecute()) {
			...
			await Promise.all(this.seedSources.map(async (source) => {
				...
				const seedPromise = writeSeeds(async (params) => dynamodb.doc.send(new BatchWriteCommand(params)), source.table, locateSeeds(source.sources || []));
				const rawSeedPromise = writeSeeds(async (params) => dynamodb.raw.send(new BatchWriteItemCommand(params)), source.table, locateSeeds(source.rawsources || []));
